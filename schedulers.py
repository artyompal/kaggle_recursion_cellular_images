...
class CosineLRWithRestarts():
    ...
    def __init__(self, optimizer, batch_size, epoch_size, restart_period=100,
                 period_inc=2, max_period=100, last_epoch=-1,
    ...
    def get_lr(self, t_cur: int) -> Any:
        eta_t = 0.5 * (1 + math.cos(math.pi * (t_cur / self.restart_period)))
        weight_decay_mul = math.sqrt(self.batch_size /
                                     (self.epoch_size * self.restart_period))

        lrs = [base_lr * eta_t for base_lr in self.base_lrs]
        weight_decays = [base_weight_decay * eta_t * weight_decay_mul
